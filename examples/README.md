...
Seven **independent** real-world apps that each install `@microcharts/react` **from npm** (`0.17.0`) and integrate it
...
## Feature matrix (0.17.0 coverage)
...
```bash
cd examples/<app>
npm install                                        # resolves @microcharts/react@0.17.0 from npm
npm run dev                                        # fixed port — see table
```

Or launch any of them from the editor via the `ex-*` entries in `.claude/launch.json`.

To point the suite at a newer release or RC, bump `@microcharts/react` in each app's `package.json` (e.g. `"0.17.1"` or
