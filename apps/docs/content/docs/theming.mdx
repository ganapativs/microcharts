...
:root {
  ...
  /* Optical lift for centred glyph marks (arrow, dot, moon…) that sit on the
     x-height seat instead of the baseline — font-relative. 0em to centre exactly
     on the x-height; the default nudges toward the cap-height midpoint. */
  --mc-glyph-nudge: -0.04em;
...
attribute — pure CSS, zero JavaScript. They change how charts _look_, never what the data _means_.
...
<Callout type="info">
  ...
  previews the same bundles live across every page — it's the exact token contract you'd use in your own app.
</Callout>

## Copy the tokens [#copy-tokens]

Everything above, in one place. Pick a style, add your brand accent, choose a mode, and copy the exact `--mc-*` block —
light and hand-tuned dark together — ready to paste into your app. The preview renders real charts in both modes and
names each role it paints, so an accent choice shows up the instant you make it. What you copy is the same contract the
library ships, never an approximation.
