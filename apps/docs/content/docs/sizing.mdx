...
element (for hover and keyboard), but that wrapper and the SVG are always the **same box** — so the same recipe fills
it, and `width` / `height` / `style` / `className` mean exactly what they do on the static entry:
...
<Callout type="info">
  Because the wrapper and the SVG share one box, the hover crosshair, highlight, and readout always land under the
  cursor — at any size, in any flex or grid cell. An interactive chart is its static twin plus interaction, never a
  different size.
</Callout>
