...
To make a chart fluid, drop its pixel size and let CSS drive the width instead. The `viewBox` keeps the aspect ratio, so
`height: auto` follows the width. This is the one-liner for a KPI card, a responsive dashboard tile, or a full-width
hero:
...
before CSS overrides the width. Utility classes work the same way (`className="w-full h-auto"`).
