...
ink doesn't. Most rectilinear marks also use `shape-rendering: crispEdges`, so bars and grids stay sharp at any size. A
mark under one viewBox unit opts back out, because snapping its edges to the pixel grid can erase it.
