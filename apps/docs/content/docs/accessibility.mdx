...
title: Accessibility
description:
  Accessible by default — every chart is an img with a natural-language summary generated from your data,
  keyboard-navigable when interactive, localizable, and correct under forced-colors, reduced-motion, and high-contrast.
...
The `/interactive` entries are focusable and fully keyboard-navigable. Arrow keys walk the points — or move in two
dimensions across an ActivityGrid — and each focused value is announced through a polite live region, so a keyboard or
screen-reader user reads the same detail a mouse user gets on hover. The static entries carry no listeners at all;
...
- **Localized words.** The summary templates are a swappable contract (`SummaryStrings`; the English set ships as the
  exported `EN` dictionary). Spread `EN` and override the keys you're translating, then pass the result via the
  interactive `strings` prop — or override the whole sentence with a `summary` string — and the announcement speaks your
  language. The generator itself is exported too: `import { describeSeries } from "@microcharts/react"` returns the
