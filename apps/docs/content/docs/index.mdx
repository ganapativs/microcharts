...
title: Introduction
description:
  Word-sized charts for React, made for LLMs and humans — zero dependencies, 0.9–3.6 kB gzip each, accessible by
  default, and server-component safe. 106 chart types, one grammar.
---
...
- **Server-component safe.** Static charts are hook-free and listener-free; they render to HTML with zero client
  JavaScript, and add interactivity only from a separate `/interactive` subpath.
...
- **Tiny.** Static charts span **0.9–3.6 kB gzip** (median 2.3), each imported from its own subpath and budget-gated in
  CI.
...
inline in a sentence. Ready to render one? The [Quickstart](/docs/quickstart) gets you from install to a live chart in
minutes.
