...
description:
  ...
  /interactive entry, a chart that won't fill its container, and fonts that look wrong — each with its one-line fix.
---
...
Two ways out: render the static default entry instead if you don't actually need hover or keyboard —
...
`width`/`height` size an interactive chart exactly the way they size its static twin, and the hover crosshair stays
locked to the cursor at any size. Full recipes, including the inline-in-a-sentence case, are on
