...
The accessible name is the state's label — **"Status: warning."** — composed with your `title` ("Deploys. Status:
warning."). The interactive entry announces state changes through a polite live region and stays quiet on mount. `pulse`
is a CSS halo that disappears entirely under `prefers-reduced-motion`.
