...
title: StarSpoke
description:
  A star glyph for React — spokes radiate from a center, length is value, and there is deliberately no connecting
  polygon, so profiles compare honestly.
---
...
instances — that fixed order, plus the always-on guide spokes, is what keeps a profile readable even where a metric's
label doesn't seat. Tip labels use the same fit-or-drop rule as the rest of the library: each one only renders if it
clears its side of the glyph without heavy clamping, so a long metric name never gets squeezed into an overlap — it
drops cleanly and the shape still reads.
