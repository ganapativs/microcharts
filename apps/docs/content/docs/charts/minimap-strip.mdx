...
title: MinimapStrip
description:
  A minimap for React — a content thumbnail with a draggable viewport window, an annotation-tick lane, and fog over
  unknown regions, so absence never reads as zero.
---
