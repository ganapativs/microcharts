...
title: EtaBar
description:
  A progress bar for React that re-forecasts honestly — the remainder is sized by the observed rate, not linear
  interpolation, so a stalling transfer shows it.
---
...
>
  <EtaBar progress={0.42} elapsed={10} rate={0.05} locale="de-DE" title="Localized ETA" width={280} height={16} />
</LiveDemo>

Without a `formatEta`, the remaining-time figure falls back to `format`/`locale` — under `de-DE` that's a decimal comma
("11,6") rather than the English decimal point ("11.6"). Pass `formatEta` (as the hero example does) to add a unit; it
...
>
  <EtaBar progress={0.08} elapsed={30} rate={0.001} title="Barely started" width={280} height={16} />
</LiveDemo>
...
The accessible name is the honest forecast — **"64% done; about 2 min remaining at the current rate."** A stalled
