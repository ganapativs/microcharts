...
title: BurnChart
description:
  A sprint burndown chart for React — plan vs actual with a dotted, honestly-provisional projection to the deadline and
  a signed landing gap.
---
...
With a `locale`, both the gap label and the accessible summary's numbers follow that locale's own grouping — "2.300" in
German, not "2,300".
...
compare it to, so the label and summary drop the "vs planned" clause. A stalled burn (`actual` flat) never reaches zero,
so `finishes` is `false` and the summary states that plainly rather than extrapolating a fake landing. A single `actual`
...
The accessible name states progress against the plan and the projected landing — **"6 of 11 days in: 21 points remain vs
