...
title: StackedArea
description: >-
  React stacked area chart — composition shift over time, capped at three series. Tiny accessible SVG microchart;
...
entry announces every layer at once (**"Point 8 of 12: Mobile 56%, Web 36%, API 8%."**), and the readout chip shows the
same breakdown as one row per band — a composition chart that named only its biggest series answered a different
