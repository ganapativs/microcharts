...
description:
  A reliability diagram for React — predicted vs observed probability against the identity diagonal, with an always-on
  support lane so tiny bins never look authoritative.
---
...
The accessible name reports the worst miscalibration and the support gaps — **"10 bins; largest gap at 0.7 predicted
(observed 0.52); 2 low-support bins."** The interactive entry roves the bins with ←/→, announcing each bin's predicted
