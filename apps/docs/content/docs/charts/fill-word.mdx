...
title: FillWord
description: >-
  React fill word chart — a word whose glyphs fill with accent — the label is the bar. Tiny accessible SVG microchart;
...
The accessible name names the task and its state — **"uploading: 62% complete."** — or, in drain mode, where `value` is
