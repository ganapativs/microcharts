...
title: FillWord
description:
  A fill-word chart for React — a word whose own glyphs fill with an accent to show progress, where the label is the
  bar.
---
...
The accessible name names the task and its state — **"uploading: 62% complete."** — or, in drain mode, **"expiring: 70%
remaining."** An empty word reads "No data." The interactive entry glides the ink edge with a reduced-motion-gated
clip-path transition and announces changes through a polite live region, throttled to at most once a second so a
streaming value never spams.
