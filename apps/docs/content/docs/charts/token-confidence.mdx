...
description:
  A confidence display for React where the text is the chart — tinted underlines beneath generated tokens flag which
  parts to double-check, in three discrete tiers.
---
...
no mark at all, and each tier is distinguished by colour **and** underline thickness **and** stroke style, so it never
depends on colour alone. For auditing exact probabilities, reach for [CalibrationStrip](/docs/charts/calibration-strip)
instead.

## Accessibility

The accessible name is the tier tally — **"13 tokens: 8 confident, 2 unsure, 3 guessing."** The interactive entry gives
