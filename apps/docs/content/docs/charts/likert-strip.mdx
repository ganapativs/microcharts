...
title: LikertStrip
description:
  A diverging Likert strip for React — valence and balance from a center line, with both neutral conventions supported
  honestly.
---
...
neutral from the bar for cleaner pole comparison — but its share is ALWAYS labeled and spoken; neutral is never silently
dropped. Graded opacity encodes ordinal distance from neutral, never magnitude.
