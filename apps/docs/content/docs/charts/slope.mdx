...
labels drop deterministically twice over: when rows are too dense (count × height) and when endpoints collide within a
font-height (greedy per-column dedup) — pure arithmetic, nothing measured. A missing end renders a dashed stub and is
announced "incomplete", never interpolated.

## Accessibility

The accessible name counts directions and leads with the biggest mover — **"3 categories: 2 up, 1 down. Largest change
West, down 25%."** The interactive entry finds the nearest line under the pointer and roves categories ordered by their
