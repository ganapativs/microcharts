...
meaningless, so the docs insist on explicit `latencyDomain` and `rateDomain` — the same steer as `FatDigits`. An alert
doubles the satellite's size and flags the summary, never relying on color alone. Unknown latency or rate goes gray and
stops the spin: an unknown dependency must not look healthy.
