...
title: ErrorBudget
description:
  An SLO error-budget chart for React — budget remaining against the steady-burn diagonal, with Google-SRE burn-rate
  reference lines and exhaustion marking.
---
...
reaches zero before `window` elapses, the line stops at that step and an ✕ replaces the endpoint dot — no further steps
are drawn, extrapolated, or implied, and the summary reads **"Budget exhausted at day 7 of 20."** instead of the normal
remaining/rate sentence.
...
zero before the window ends, the line stops at an ✕ and the summary says so.
