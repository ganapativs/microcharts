...
title: DicePips
description:
  A dice-pips chart for React — canonical die faces 1–6 read instantly by subitizing, with an honest numeral fallback
  past six.
---
...
interactive entry cross-fades the pip set on change and announces the new face through a polite live region; the pips
are one value, so there is no cursor to move.
