...
title: MicroScatter
description:
  A word-sized scatterplot for React — 2-D position with an honest Pearson r in the summary and an optional
  least-squares trend.
---
...
Axes are unlabeled at this scale, so `title` must name both variables — the examples here all follow that convention.
The summary's relationship words are a documented heuristic on |r| (≥ 0.7 strong, ≥ 0.4 moderate, ≥ 0.2 weak), and
whenever a relationship word appears, r appears beside it: claim and evidence travel together. Fewer than 3 points, or a
