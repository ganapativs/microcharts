...
description:
  A two-level icicle strip for React — parent shares on top, children aligned exactly underneath, so a hierarchy stays
  legible at 24px where a treemap loses alignment.
---
...
The accessible name names the biggest leaf and its parent — **"4 groups, 5 parts; largest JS → react (28% of the
