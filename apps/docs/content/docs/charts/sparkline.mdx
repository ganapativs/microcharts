...
title: Sparkline
description: >-
  React sparkline chart — tiny accessible SVG trend line. RSC-safe static entry, or /interactive with hover and
  keyboard. Zero-dependency microchart from @microcharts/react.
---
...
Pass a `title` to name it, or `summary={false}` — with no `title` — to make it decorative when the surrounding text
already says what it shows. The interactive entry adds a polite live region that reads each focused point as you arrow
through it.
