...
title: CyclePlot
description:
  A cycle plot (seasonal-subseries chart) for React — a slot-mean spine plus raw within-slot micro-trends, so
  seasonality and drift stay separate reads.
---
...
A slot with no finite values in it draws no mean tick and no within-slot line; the spine connects only the non-empty
slots, so it breaks cleanly around the gap rather than interpolating across it. A single cycle (one value per slot)
draws the spine and dots as usual, but no within-slot polylines — a line needs at least two observations in a slot.
...
The `center` choice is explicit — mean by default, median when a slot's distribution is skewed — and the summary names
which one it used.
