...
title: ParetoStrip
description:
  A Pareto chart for React — descending bars plus a cumulative-share line on a fixed 0–100% scale, with the accent
  stopping at the threshold crossing.
---
...
always last and never counted among the vital few; if it towers over the top cause, that usually means `max` is set too
aggressively.

## Accessibility

The accessible name states the vital-few count and its cumulative — **"Top 4 of 9 causes account for 82% of
incidents."** — or, without a threshold, the top cause and its share. The interactive entry steps the bars and announces
each bar's share and running cumulative; **T** jumps to the crossing bar.
