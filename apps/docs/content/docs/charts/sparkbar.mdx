...
title: SparkBar
description:
  Compact bars for discrete magnitudes in React, or a win–loss–tie streak of outcomes — static SVG and an interactive
  entry.
---
...
appears, so direction survives forced-colors and never depends on color alone. The endpoint bar gets accent emphasis by
