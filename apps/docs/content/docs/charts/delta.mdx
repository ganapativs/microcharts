...
title: Delta
description:
  An inline signed change for React KPIs — direction double-encoded by glyph and color, with optional
  percent-from-prior.
---

Delta is a text-first metric: a signed number with a direction glyph. It reads inline, next to a KPI, or inside a table
cell. Direction is always doubled — a triangle _and_ a color — so it never relies on color alone.
...
`format` also takes `Intl.NumberFormatOptions` — with a `locale`, the sign and magnitude follow that locale's own
grouping and decimal marks (a German reader sees a comma decimal and a space before the `%`, not a period).
