...
title: Hourglass
description:
  An hourglass chart for React — sand area splits elapsed and remaining, area-true, for the two-sided time story
  Progress can't tell.
---
...
The accessible name carries both sides — **"75% elapsed, 25% remaining."** The interactive entry cross-fades the sand
levels on change (an opacity swap, never a path interpolation) and announces only when the total crosses a documented
threshold (50 / 90 / 100%), so a streaming value never spams.
