...
title: HeatStrip
description: >-
  React heat strip chart — ordered intensity along a single strip for dense series. Tiny accessible SVG microchart;
...
The summary reuses `describeSeries` verbatim — for the load strip above that's **"Trending up 192%. Range 12 to 90. Last
value 35."** — so a color ramp is never the only channel. The interactive entry roves cells with ActivityGrid-parity
