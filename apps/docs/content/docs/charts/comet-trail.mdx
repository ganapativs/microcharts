...
title: CometTrail
description:
  A comet trail chart for React — a live value's head dot plus an opacity-fading trail of where it has just been.
---
...
`prefers-reduced-motion` the head repositions instantly (the static encoding is already complete), and the sweep pauses
off-screen.
...
The accessible name is the now-value and the recent trend — **"Now 87, rising over the last 12 updates."** — or **"Now
87."** for a single point. Arrow keys step back through the trail ("3 updates ago: 78.") and return toward now; motion
is gated on `prefers-reduced-motion`, and the trail opacity is age, never a value you must read from fading.
