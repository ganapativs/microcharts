...
description:
  An ensemble / hypothetical-outcome chart for React — a faint bundle of simulated paths with one emphasized
  representative, so futures that disagree in shape stay visible.
---
...
draws that synthetic path instead, and the summary calls it out). Selection and emphasis are deterministic — nothing
about this chart varies between renders of the same data.
...
The accessible name states the endpoint spread and the typical path — **"8 simulated paths end between 28 and 61;
typical path ends near 46."** The interactive entry flips through the members (the HOP loop) on hover; with reduced
