...
The ledger range is the default because clipping pitch silently is worse than two extra hairline ledger ticks — a note
above or below the staff still reads truly. `range="staff"` exists for dense cells where ledger lines would collide with
...
4." The sonification idea (playing the pitches) is one paragraph of future work; no audio ships today.
