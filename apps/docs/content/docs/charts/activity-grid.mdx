...
title: ActivityGrid
description: >-
  React activity grid chart — contribution-graph intensity on a calendar or matrix. Tiny accessible SVG microchart;
...
announcing each cell's value as you move. Give it an `anchor` and every cell is a dated day, so the readout names the
date rather than a position ("Mar 2: 12").
