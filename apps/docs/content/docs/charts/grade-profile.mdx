...
title: GradeProfile
description:
  A word-sized route profile for React — elevation ridge with pitches coloured by grade, so the hard climbs read at a
  glance.
---
...
`format` accepts `Intl.NumberFormatOptions` (with a `locale`) or a formatting function, and applies to the distance and
elevation numbers in the summary and readout. Grades carry their own percent formatting, so the locale still decides the
...
so a missing reading never invents a grade. Flat routes and single points behave the same honest way.
...
The accessible name states the route's shape in words — **"900 m, 67 m gain; steepest 16% at 800 m."** — distance, total
