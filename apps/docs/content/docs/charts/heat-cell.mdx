...
title: HeatCell
description:
  One calibrated color step for React tables — the building block for host-owned intensity grids with a shared domain.
---
...
The accessible name carries the value _and_ its calibration — **"42 — level 3 of 5."** — so the color scale is never the
only channel. Non-finite input renders a designed empty track and says **"No data."** The interactive entry reveals the
same reading on hover/focus, with ActivityGrid announcement parity.
