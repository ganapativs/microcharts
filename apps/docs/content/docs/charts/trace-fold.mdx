...
title: TraceFold
description:
  A folded micro-trace for React — one rect per span by start, duration, and depth, with the critical path accented so
  "where did the latency go" reads without a console.
---
...
The accessible name names the biggest span and its path status — **"9 spans over 214 ms; longest db.query (86 ms) on the
