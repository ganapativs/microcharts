...
title: ForecastCone
description:
  A fan chart for React — history plus a widening prediction cone with a dashed median, at most two bands, and an
  optional target line to clear.
---
...
`format` also takes `Intl.NumberFormatOptions` — with a `locale`, the accessible summary's median, interval, and target
numbers follow that locale's own grouping and currency placement ("4.200 €" in German, not "€4,200"). The in-chart
landing label stays the plain formatted number either way; only the announced numbers change shape.
...
clears it: **"Median forecast 42 by week 11 (80% between 33 and 55), from 38 today. The 80% band straddles the 45
