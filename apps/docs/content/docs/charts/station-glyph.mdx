...
title: StationGlyph
description:
  A meteorological station model for React — sky cover, wind, temperature, dew point, and pressure packed into one
  glyph, each on its own honest channel, no legend.
---
...
a barb count reads exactly, a tiny length does not. Absent fields simply don't render; the glyph never invents a zero.

## Accessibility

The accessible name is the whole observation — **"KSFO, wind southwest 15; sky broken, 16° / 9°, 1,013."** The
interactive entry roves the fields with ←/→ so a screen-reader user can step through station, wind, sky, temperature,
dew point, and pressure one at a time, and Home returns to the full reading.
