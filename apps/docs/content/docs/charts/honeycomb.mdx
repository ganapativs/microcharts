...
title: Honeycomb
description:
  A honeycomb chart for React — filled cells of a hex grid for occupancy of a capacity, countable up to about sixty.
---
...
The accessible name is the exact occupancy — **"34 of 40 seats filled."** — always the true value, even when it exceeds
the total. The interactive entry announces the count on change through a polite live region and reveals the value /
total on hover; the cells are anonymous units, so there is no per-cell cursor to move.
