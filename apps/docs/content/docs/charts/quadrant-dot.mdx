...
title: QuadrantDot
description:
  A 2×2 quadrant chart for React — a focal dot placed against a peer field, with a split cross and axis-relative
  summaries. A position read, not a scatter plot.
---
...
non-finite focal position renders nothing but the frame, with the "No data." summary — the same degenerate path an empty
series takes elsewhere in the library.
...
The accessible name states the focal position, the axis-relative quadrant, and how crowded that quadrant is — **"Impact
9, effort 3 — in the high-impact, low-effort quadrant (2 of 14 peers)."** — or your own words via
`quadrants={["quick win", …]}`. The interactive entry cycles the peers nearest-first, each announced with its
