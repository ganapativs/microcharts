...
title: Progress
description:
  A zero-anchored progress bar for React — the percent label is the datum, with segments for step counts and honest
  >100% handling.
---
...
`max <= 0` (or a non-finite `value`/`max`) renders an empty track with no fill and no percent label — the accessible
name says "No data." instead of computing a meaningless ratio. A `value` of `0` renders a real, empty-but-present bar
(not the no-data state), so "not started" and "unmeasurable" stay visually distinct.
