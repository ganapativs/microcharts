...
- https://microcharts.dev/catalog.json — full API. Do **not** read the whole file. Find the chart by `slug`, then use
  ...
  those; otherwise use the chart's `.md` for interactive callbacks. Read that chart's `gotchas` before writing props.
  Never invent props.
...
- These charts are word-sized. `width`/`height` are viewBox units with an authored maximum per chart (`maxWidth` /
  `maxHeight` in `/catalog.json`); stay inside it and scale with CSS beyond it, because past that box the geometry stops
  scaling and the extra room is whitespace.
...
If your app declares its own cascade layers (Tailwind v4 does), the import order of this stylesheet decides whether your
utility classes or the library's rules win on a chart. The mechanism and both fixes are in
[Theming → Where the library's rules sit in the cascade](/docs/theming#where-the-librarys-rules-sit-in-the-cascade).
