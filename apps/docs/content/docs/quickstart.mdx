...
That's the whole setup: the agent installs `@microcharts/react`, imports the stylesheet, and writes your conventions
file — nothing left for you to wire up. Skip straight to [your first chart](#your-first-chart). Not using an agent? The
same two steps by hand are below.

## Set up by hand

The two steps the agent runs for you — do them once, at your app root, and you're set.
...
The interactive entry **is** a client component: in a Next.js App Router page, render it from a file with `"use client"`
at the top (or anywhere inside one).
...
nothing to rove between. Those single-value charts also take `live` (default `true`): as the value updates, the new
reading is announced through a polite live region, throttled so a streaming number never spams a screen reader. Two
charts opt out of the picker on purpose: `MinimapStrip` is a range slider with its own `onWindowChange([lo, hi])`, and
`TokenConfidence` moves real focus through inline text spans. Full detail, including the live-region behavior, is on
