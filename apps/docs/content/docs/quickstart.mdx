...
- These charts are word-sized. `width`/`height` are viewBox units with an authored maximum per chart (`maxWidth` /
  `maxHeight` in `/catalog.json`); stay inside it and scale with CSS beyond it, because past that box the geometry
  stops scaling and the extra room is whitespace.
