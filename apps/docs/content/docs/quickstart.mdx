...
<Callout type="info">
  **Does your assistant speak MCP?** Add the server instead of pasting the prompt, and it can search the catalog and
  ...
  reference are in [MCP server](/docs/mcp) — the two are complementary, not alternatives: the prompt wires the library
  into your repo, the server answers chart questions while you work.
</Callout>
