...
description:
  ...
  1–3.7 kB gzip (median 2.3) and rendered to SVG at about 0.031 ms each. Every number here is reproducible from one
  command.
---
...
The whole catalog lands between **0.95 kB and 3.67 kB** gzip, with a **median of 2.33 kB** — and that's the _static_
entry. Interactivity is opt-in from a separate `/interactive` subpath and adds about 1.9 kB at the median (median 4.2 kB
for the interactive entry) for the whole contract — hover, roving keyboard focus, select-to-pin, touch scrub, and live
announcements — so a page pays for it only where it actually uses it. About 0.5 kB of that is the shared picker kernel,
which the per-subpath gate measures standalone in every entry; a page using several interactive charts bundles it once.

<SizeTable />

Nine charts sit above the 3 kB reference line. Sparkline, the most configurable primitive in the set, is the largest;
the rest are the value-series charts that can host annotations (ChangePoint, WinProbWorm, ForecastCone, DualSparkline,
BurnChart, SpreadBand) plus Station Glyph and Slope, which share one layout pass between their static and interactive
entries so overlays can never drift from the marks. The annotation hosts carry a small shared walker that resolves
`Threshold`, `Marker`, and the rest against the chart's scale — you pay for it only on those charts, and each is still
smaller than a single icon font.
