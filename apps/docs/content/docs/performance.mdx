...
The `/interactive` subpath lands between **2.17 kB and 6.75 kB** gzip, with a **median of 5.07 kB**. That buys hover,
roving keyboard focus, select-to-pin, touch scrub, and live announcements. Its static twin is pure SVG with zero client
JavaScript, and lands between **1 kB and 4.07 kB**, with a **median of 2.64 kB**. A page pays for interactivity only
...
that resolves `Threshold`, `Marker`, and the rest against the chart's scale, and you pay for it only on those charts.
...
The summary kernel is just as cheap: `describeSeries` runs at roughly **892,000 calls per second** on a 24-point series,
