...
description:
  ...
  interactive · ~1–4 kB static, static SSR median about 0.03 ms each. Every number here is reproducible from one
  command.
---
...
The interactive catalog lands between **2.17 kB and 6.76 kB** gzip, with a **median of 5.08 kB** — hover, roving
keyboard focus, select-to-pin, touch scrub, and live announcements from the `/interactive` subpath. The static twin
lands between **0.99 kB and 4.06 kB**, with a **median of 2.64 kB** — pure SVG, zero client JavaScript. A page pays for
...
Twenty-seven charts sit above the 3 kB static reference line, none of them by more than 1.06 kB. Sparkline, the most
...
PolarClock, Constellation, Dumbbell, CyclePlot, Waterfall, TapeGauge, GradedBand and LikertStrip — the last two crossed
the line when their percent labels moved onto a real `Intl` formatter. The annotation hosts carry a small shared walker
that resolves `Threshold`, `Marker`, and the rest against the chart's scale — you pay for it only on those charts, and
each is still smaller than a single icon font.
...
to hydrate. Across all 106 chart types the median **static** SSR render is about **0.03 ms per chart** — the heavier
