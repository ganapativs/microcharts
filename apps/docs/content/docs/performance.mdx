...
The whole catalog lands between **0.95 kB and 3.74 kB** gzip, with a **median of 2.34 kB** — and that's the _static_
entry. Interactivity is opt-in from a separate `/interactive` subpath and adds about 1.9 kB at the median (median 4.22
kB for the interactive entry) for the whole contract — hover, roving keyboard focus, select-to-pin, touch scrub, and
live announcements — so a page pays for it only where it actually uses it. About 0.5 kB of that is the shared picker
kernel, which the per-subpath gate measures standalone in every entry; a page using several interactive charts bundles
it once.
