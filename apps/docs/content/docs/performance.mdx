...
The `/interactive` subpath lands between **2.27 kB and 6.98 kB** gzip, with a **median of 5.31 kB**. That buys hover,
roving keyboard focus, select-to-pin (and light dismiss), touch scrub, and live announcements. Its static twin is pure
SVG with zero client JavaScript, and lands between **1.08 kB and 4.26 kB**, with a **median of 2.78 kB**. A page pays
...
Thirty-three charts sit above the 3 kB static reference line, none of them by more than 1.26 kB. Sparkline, the most
...
EventTimeline, VolumeProfile, GradedBand, LikertStrip and MicroDonut. GradedBand and LikertStrip crossed the line when
their percent labels moved onto a real `Intl` formatter; MicroDonut crossed it when its total label gained a fit gate
against the hole it sits in. The annotation hosts carry a small shared walker that resolves `Threshold`, `Marker`, and
