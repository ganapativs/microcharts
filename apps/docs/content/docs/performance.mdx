...
title: Performance
description:
  Zero dependencies, budget-gated sizes, and server rendering with no client JavaScript — 106 chart types measured at
  1–3.9 kB gzip (median 2.5) and rendered to SVG at about 0.031 ms each. Every number here is reproducible from one
  command.
---
...
The whole catalog lands between **0.95 kB and 3.93 kB** gzip, with a **median of 2.51 kB** — and that's the _static_
entry. Interactivity is opt-in from the `/interactive` subpath, which runs a median 4.7 kB — about 2.2 kB more — for the
whole contract: hover, roving keyboard focus, select-to-pin, touch scrub, and live announcements. A page pays for it
only where it actually uses it. About 0.5 kB of that is the shared picker kernel, which the per-subpath gate measures
standalone in every entry; a page using several interactive charts bundles it once.

<SizeTable />

Twenty charts sit above the 3 kB reference line, none of them by more than 0.93 kB. Sparkline, the most configurable
primitive in the set, is the largest; most of the rest are the value-series charts that can host annotations
(ChangePoint, WinProbWorm, ForecastCone, DualSparkline, BurnChart, SpreadBand, RetentionCurve, ControlStrip,
PercentileLadder, QueueDepth, SparkBar, ShiftHistogram, StackedArea, ABStrips), plus Station Glyph, Slope, PolarClock,
Constellation, and TapeGauge, which share one layout pass between their static and interactive entries so overlays can
never drift from the marks. The annotation hosts carry a small shared walker that resolves `Threshold`, `Marker`, and
the rest against the chart's scale — you pay for it only on those charts, and each is still smaller than a single icon
font.
...
to hydrate. Across all 106 chart types the median render is about **0.031 ms per chart** — the heavier expressive and
frontier instruments cost more geometry, the simple strips cost almost nothing.
