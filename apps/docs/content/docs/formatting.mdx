...
StackedArea, OHLC, LikertStrip, IconArray, ConfusionGrid, EventTimeline, BreathingDot, and Honeycomb. The other four
set a different default: BiasStrip shows the sign on every non-zero bias, VolumeProfile uses compact notation, and
DualWindowMeter and HistogramStrip pin their decimal places.

Your `format` merges with those defaults key by key rather than replacing them, so `format={{ notation: "compact" }}`
on a percent chart still writes a percent.
