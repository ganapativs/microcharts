...
## Your format merges with the chart's unit

Fifteen charts format in a unit of their own: Funnel, Progress, ProgressRing, StackedArea, OHLC, LikertStrip,
IconArray, ConfusionGrid, EventTimeline, BreathingDot, BiasStrip, Honeycomb, DualWindowMeter, VolumeProfile, and
HistogramStrip all label a share as a percent. On those charts your `format` merges with that unit key by key rather
than replacing it, so `format={{ notation: "compact" }}` on a percent chart still writes a percent.

Passing an explicit `style` opts out of the unit, and it drops the chart's digit defaults with it. Those defaults are
calibrated for the unit: `maximumFractionDigits: 0` reads as `3%`, and kept under `style: "decimal"` it rounds 0.03 to
`0`. Changing the unit voids the whole default rather than half of it.
