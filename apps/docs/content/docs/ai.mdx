...
needs the finished mark, not an instruction to draw one.

[`@microcharts/mcp`](/docs/mcp) is that third verb. It's a Model Context Protocol server, run over stdio on your own
machine, that gives an assistant three tools: **find** the chart type that answers a question, **get** its exact props
and a valid sample, **render** it to a self-contained SVG with the real generated alt text attached. The rendering is
done by this library, so what comes back is the same mark you'd get from the component — honest by construction, not a
picture of one.
