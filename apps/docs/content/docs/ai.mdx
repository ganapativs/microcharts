...
this, and it's about forty lines.
...
the chart ships as markup. Add `/interactive` only where you need hover or keyboard.
...
Need a chart in a README, an email, or a non-React runtime? Today, render the static component to a string on the
server. A first-class JSON spec and a React-free string renderer are on the roadmap for rendering to SVG with no React
at all.
...
the [gallery](/gallery) shows them at a glance. Everything an agent needs to emit correct microcharts fits on one card —
copy it straight into a system prompt:
