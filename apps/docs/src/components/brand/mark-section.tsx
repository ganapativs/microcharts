...
export function BrandMarkSection() {
  return (
    <section className="mx-auto max-w-shell px-4 py-14 sm:px-6">
      <SectionMark>The mark</SectionMark>
      <Reveal className="mb-8 max-w-2xl">
        <h2 className="display text-[length:var(--text-fluid-h2)]">Where the mark comes from</h2>
      ...
      <Reveal delay={60}>
        <dl className="panel-soft grid grid-cols-2 sm:grid-cols-4">
          {SPECS.map(([k, v], i) => (
            <div
              key={k}
              className={
