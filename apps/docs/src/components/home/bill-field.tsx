 ...
 *
 * `oneChartGzipKb` units, each one kilobyte. The first few are inked (what a
 * microchart costs) and the rest ghosted (what you don't ship), so the comparison
 * is counted rather than asserted. Running off the right edge is the point: the
 * bill does not fit inside the measure.
 *
 * Every measured figure in this section states itself; only the provenance line
 * at the bottom links to where the numbers came from.
 */
    ...
    <>
      ...
      >
        ...
        <div className="units pt-2">
          ...
          >
            {/* One template literal: SWC glues the figure to the next word when a
                text node follows an expression, carries an entity and spans a
                newline (swc-ssr-spaces.test.ts). */}
      ...
      <div className="shell u-block grid items-start gap-x-10 gap-y-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,224px)] lg:gap-x-16">
        <div className="grid gap-6">
          ...
          <p className="lead" style={{ maxWidth: "var(--m-lead)" }}>
            ...
            <span className="num">0</span> kB of client JS.
          </p>
          ...
          >
            ...
            <a href={SIZE_LIMIT} target="_blank" rel="noreferrer noopener" className="ulink">
