 ...
 *
 * **1. Element-scoped presets** (`[data-home-preset]`). The site keys presets off
 ...
 *
 * **2. The inverted sheet** (`[data-invert]`). A chart on the opposite stock
 ...
 *
 * Nothing here is a copied colour. `home.test.ts` fails on a hex literal in
...
/**
 * Valence and reference ink, deepened toward the sheet's own text ink. The
 * library's per-mode values are tuned for its own field; this page inverts onto a
 * lighter light sheet and a deeper dark one, where the mid-tone inks fall under
 * the 3:1 bar for a graphical object. 30% measured across all six presets × both
 * sheets: minimum 3.4:1.
 */
...
export function PresetScopeStyle() {
  ...
  for (const preset of PRESETS) {
    ...
    const sel = `[data-home-preset="${preset.id}"]`;
  ...
  for (const preset of PRESETS) {
    ...
    blocks.push(`${root} .home [data-invert]{${decls(onDarkSheet)}}`);
    blocks.push(`${root}.dark .home [data-invert]{${decls(onLightSheet)}}`);
  }
  ...
  for (const accent of ACCENTS) {
    ...
    blocks.push(`:root${sel} .home [data-invert]{${accentDecl(accent.dark)}}`);
    blocks.push(`:root${sel}.dark .home [data-invert]{${accentDecl(accent.light)}}`);
  }
  // Cobalt is the site default and carries no [data-accent] attribute.
  const base = ACCENTS[0]!;
  blocks.push(`:root:not([data-accent]) .home [data-invert]{${accentDecl(base.dark)}}`);
  blocks.push(`:root:not([data-accent]).dark .home [data-invert]{${accentDecl(base.light)}}`);
  ...
  for (const id of ownsAccent) {
    ...
    blocks.push(`:root${sel} .home [data-invert]{${accentDecl(accentOf(dark))}}`);
    blocks.push(`:root${sel}.dark .home [data-invert]{${accentDecl(accentOf(light))}}`);
  }
