...
export function GiveUpTable() {
  ...
  return (
    <div className="shell u-sub">
      ...
      <div className="mt-5">
        <table
          className="compare w-full border-collapse font-mono text-[12.5px] leading-[1.5] tracking-[-0.03em]"
            ...
            {rows.map(([measure, ours, theirs]) => (
              <tr key={measure}>
                ...
                {/* `data-col` is the mobile row label: below 700px the table
                    restacks (see home.css) and the head row is gone. */}
                <td
      ...
      {/* `full-chart-libraries`, not `vs-recharts`: that page argues one
          decision, and this line is conceding the other one. */}
      <p className="prose mt-6" style={{ maxWidth: "var(--m-prose)" }}>
        If you need axes and a legend, use{" "}
        <Link prefetch={false} href="/docs/full-chart-libraries" className="ulink">
          a toolkit
        </Link>
