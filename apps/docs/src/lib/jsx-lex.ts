 ...
 * it. The token classes (`tok-*`) live in global.css and ride the theme, so a
...
export function lexJsx(code: string): Tok[] {
  ...
  for (const m of code.matchAll(TOK_RE)) {
    ...
    if (/^<\/?[A-Z]/.test(t) || t === "/>" || t === ">") cls = "tok-tag";
    else if (/^[A-Za-z]/.test(t) && code[(m.index ?? 0) + t.length] === "=") cls = "tok-attr";
    else if (t.startsWith('"')) cls = "tok-str";
