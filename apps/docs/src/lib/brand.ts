...
/** How far the squircle's ink is inset from its 32-unit box on every side: the
 *  superellipse spans 2.4 → 29.6, so it fills 85% of the box. Anything seating
 *  the mark against type — the footer wordmark stands it on the baseline like a
 *  letter — has to work in ink, not in box. */
export const SQUIRCLE_INSET = 2.4 / 32;

export const CELL_SIZE = 4;
export const CELL_R = 1.2;

/** Warm near-white — reads cleanly on every accent (cobalt, ember, teal…). */
export const CELL_FILL = "#faf7f1";

/**
 * Climbing cells: bottom-left → top-right, fill graded faint → solid. Centred on
 * (16,16) matching the squircle, with even 1-unit diagonal gaps; every cell sits
 * 6.6 units from the squircle edge (measured), so the mark is symmetric.
 */
export const CELLS = [
  { x: 9, y: 19, o: 0.4 },
  { x: 14, y: 14, o: 0.7 },
