...
export function rollup(
  ...
  if (positive.length <= maxSegments) {
    return positive.map((d) => ({ label: d.label, value: d.value, members: 1 }));
  }
  const sorted = [...positive].sort((a, b) => b.value - a.value);
  const keep = new Set(sorted.slice(0, maxSegments - 1));
...
export function largestRemainderPercents(shares: readonly number[]): number[] {
  // normalize first: callers may pass 2-dp-rounded shares whose sum drifts
  // slightly off 1, which would break the exactly-100 invariant
