...
1. **`animate` is not a prop** *on static entries*. §1's sketch predates the static-first split: motion ships only in `…/interactive` client entries + CSS (reduced-motion-gated), server output is static. The grammar's motion family is the *entry choice*, not a boolean.
   **Amended 2026-07-11 (unified-motion):** interactive entries DO take `animate?: boolean` (default `false`) — one opt-in entrance per chart through the shared engine (`src/shared/motion-gate.ts` + `motion-engine.ts`, 8 archetypes, one easing/duration/stagger vocabulary). The engine is a separate import-once subpath `@microcharts/react/motion` (styles.css-shaped DX; self-registers; `sideEffects`-listed) so un-animated consumers carry only the ~0.3 kB gate. `animate` is inert on the server, on hydrated server HTML (hydration latch — entrances play only on fresh client mounts, e.g. streamed AI UIs), under `prefers-reduced-motion`, and in hidden documents; the at-rest DOM is byte-identical to the static render (tested). Static entries remain untouched — rule 1 stands for them verbatim.
