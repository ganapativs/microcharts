...
| **T1 Informative (default)** | Chart carries information alone | **Amended (Phase 2 review):** default = deterministic composed `aria-label` (+ id-less `<title>` for hover/secondary naming); an explicit stable `id` prop opts into the maximal `<title>/<desc>` + `aria-labelledby` wiring. Rationale: generated ids in hook-free static components (module counters) desync under StrictMode/concurrent renders → hydration mismatches; determinism beats the marginal legacy-AT robustness of labelledby (Safari quirks fixed since 2021). ARIA 1.2 marks `img` children presentational (SHOULD-level). |
