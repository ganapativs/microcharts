...
5. **Motion restraint.** Animate only on data change and entrance — a sparkline updating should feel like "a fact being corrected, not a flourish." 150–250 ms, ease-out or linear. No idle loops, no bounce on KPI numbers. Full spec in `07-performance.md` + `08-accessibility.md`.
   **AMENDMENT 2026-07-08 (motion-as-encoding carve-out, plan/24 batch-3 conflict resolution):** continuous/looping motion is permitted ONLY in the four expressive living types whose documented primary encoding channel IS motion (HeartbeatBlip, BreathingDot, CometTrail, OrbitStatus — plan/15 E14–E17), ONLY in their `'use client'` interactive entries, always reduced-motion-gated with a meaningful static equivalent, and never as decoration on any other chart. The "no idle loops" rule stands everywhere else — a loop that does not encode live data remains forbidden and non-themeable. Audit entry in plan/12.
